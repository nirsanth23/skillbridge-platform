import { Briefcase, Building2, GraduationCap, LogOut, Sparkles, User, UserCheck } from 'lucide-react'
          ...
          <div className="flex items-center space-x-4">
            {isAuthenticated && user ? (
              <>
                {/* User Role Badge */}
                {roleMeta && (
                  <div className={`hidden sm:flex items-center space-x-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${roleMeta.bg}`}>
                ...
                <Link
                  to="/profile"
                  className="flex items-center space-x-2.5 p-1.5 pr-3 rounded-full hover:bg-slate-100 transition border border-transparent hover:border-slate-200"
                >
                  {user.profileImage ? (
                    <img
